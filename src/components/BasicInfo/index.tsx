...
export default function BasicInfo({
  ...
  useEffect(() => {
    if (releaseDatesList.length) {
      const res = releaseDatesList.filter(
        (data: ReleaseDates) => data.iso_3166_1 === "KR"
      )[0];
      if (res) {
        setKRReleaseDate(
          res.release_dates.filter((data: ReleaseDate) => data.type !== 1)[0]
        );
      }
    }
  }, [releaseDatesList]);
  ...
  useEffect(() => {
    if (releaseDatesList.length) {
      const res = releaseDatesList.filter(
        (data: ReleaseDates) => data.iso_3166_1 === productionCountry
      )[0];
      if (res) {
        setORReleaseDate(
          res.release_dates.filter((data: ReleaseDate) => data.type !== 1)[0]
  ...
  return (
    <Container>
      ...
      <Text>
        {KRreleaseDate.release_date
          ? `${KRreleaseDate.release_date.slice(0, 10)}(KR)`
          : `${ORreleaseDate.release_date.slice(0, 10)}(${productionCountry})`}
      </Text>
      <Dot>·</Dot>
